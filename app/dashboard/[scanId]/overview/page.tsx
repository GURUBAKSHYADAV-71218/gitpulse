  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[auto_1fr]">
        ...
        <Card>
          <CardHeader><p className="font-display text-sm font-medium text-ink">Category breakdown</p></CardHeader>
          <CardBody><CategoryScoreChart categories={scan.health.categories} /></CardBody>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          value={scan.securityAnalysis?.dataAvailable ? String(scan.securityAnalysis.counts.critical + scan.securityAnalysis.counts.high + scan.securityAnalysis.counts.medium + scan.securityAnalysis.counts.low) : "Unavailable"}
        ...
        <MetricCard
          ...
          hint={scan.repositoryActivity?.daysSinceLastPush !== null && scan.repositoryActivity?.daysSinceLastPush !== undefined ? `Last push ${scan.repositoryActivity.daysSinceLastPush}d ago` : undefined}
          icon={GitBranch}
        />
        <MetricCard
          label="Contributors"
          value={scan.contributorAnalysis?.dataAvailable ? String(scan.contributorAnalysis.totalContributors) : "N/A"}
          hint={scan.contributorAnalysis?.dataAvailable ? `Top: ${scan.contributorAnalysis.topContributorConcentration}%` : undefined}
          icon={Users}
        />
      </div>
      ...
      <Card>
        ...
        <CardBody>
          ...
          {scan.engineeringSummary.generatedBy === "deterministic" && (
            <p className="mt-3 text-xs text-ink-faint">Generated from deterministic analysis. Configure AI_API_KEY for an AI-written narrative summary.</p>
          )}
        </CardBody>
      </Card>
    </div>
