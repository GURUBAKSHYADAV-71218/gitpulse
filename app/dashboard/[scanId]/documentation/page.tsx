  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[auto_1fr]">
        <Card><CardBody className="flex flex-col items-center justify-center gap-2 py-8"><ScoreRing score={docsScore?.score ?? null} size={140} label="Documentation Health" /></CardBody></Card>
        <Card>
          <CardHeader><p className="font-display text-sm font-medium text-ink">Documentation checklist</p></CardHeader>
      ...
      {docs.hasReadme && docs.readmeLength !== null && (
        <Card>
          <CardHeader><p className="font-display text-sm font-medium text-ink">README overview</p></CardHeader>
          <CardBody>
            <p className="text-sm text-ink-muted">README is approximately <span className="font-mono text-ink">{docs.readmeLength.toLocaleString()}</span> characters long. GitPulse checks for structural signals like installation and usage sections; it does not evaluate writing quality.</p>
          </CardBody>
        </Card>
      )}
    </div>
