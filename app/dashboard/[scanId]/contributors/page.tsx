  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[auto_1fr]">
        <Card><CardBody className="flex flex-col items-center justify-center gap-2 py-8"><ScoreRing score={contributorScore?.score ?? null} size={140} label="Contributor Health" /></CardBody></Card>
      ...
      {contributors.concentrationRisk === "high" && (
        <div className="rounded-md border border-status-medium/30 bg-status-medium/5 px-4 py-3 text-sm text-ink-muted">
          Most repository activity is concentrated among a small number of contributors, which can increase continuity risk. This does not necessarily mean the project is poorly maintained.
        </div>
      )}
      ...
      {contributors.topContributors.length > 0 ? (
        <>
          <Card>
            ...
            <CardBody><ContributorBarChart contributors={contributors.topContributors} /></CardBody>
          </Card>

          <Card>
            <CardHeader><p className="font-display text-sm font-medium text-ink">Top contributors</p></CardHeader>
            <CardBody>
              <div className="divide-y divide-line-subtle">
                {contributors.topContributors.map((c) => (
                  <div key={c.login} className="flex items-center justify-between py-2.5">
                    <a href={c.profileUrl} target="_blank" rel="noreferrer" className="font-mono text-sm text-ink hover:text-brand hover:underline">{c.login}</a>
        ...
        <UnavailableNote reason="No contributor data was returned for this repository." />
      )}
    </div>
  );
}
