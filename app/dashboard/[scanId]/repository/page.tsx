  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[auto_1fr]">
        <Card><CardBody className="flex flex-col items-center justify-center gap-2 py-8"><ScoreRing score={repoScore?.score ?? null} size={140} label="Repository Activity" /></CardBody></Card>
        <Card>
          <CardHeader><p className="font-display text-sm font-medium text-ink">{metrics.fullName}</p></CardHeader>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        ...
        <MetricCard label="Latest release" value={metrics.latestReleaseTag ?? "None"} hint={metrics.latestReleaseDate ? formatDate(metrics.latestReleaseDate) : undefined} icon={Tag} />
      ...
      <Card>
        <CardHeader><p className="font-display text-sm font-medium text-ink">Recent activity</p></CardHeader>
        <CardBody>
          {activity?.dataAvailable ? (
            <div className="space-y-2 text-sm text-ink-muted">
              <p>Last push was <span className="font-mono text-ink">{activity.daysSinceLastPush} days</span> ago.</p>
              {activity.recentCommitCount30d !== null && <p><span className="font-mono text-ink">{activity.recentCommitCount30d}</span> commits on the default branch in the last 30 days.</p>}
              <p className="text-ink-faint">Note: popularity metrics like stars and forks are tracked separately from engineering activity — a popular project is not automatically a healthy one.</p>
            </div>
          ) : (
