...
export function ScoreRing({ score, size = 168, strokeWidth = 10, label }: { score: number | null; size?: number; strokeWidth?: number; label?: string }) {
  ...
  return (
    <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90">
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="#1C222C" strokeWidth={strokeWidth} />
        <circle
      ...
      <div className="absolute flex flex-col items-center justify-center">
        <span className="font-mono text-4xl font-semibold tabular text-ink">{score === null ? "—" : score}</span>
