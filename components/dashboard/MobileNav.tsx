...
import { LayoutGrid, Package, ShieldCheck, Wrench, GitBranch, Users, FlaskConical, FileText, ListChecks, FileOutput } from "lucide-react";
...
export function MobileNav({ scanId }: { scanId: string }) {
  ...
  return (
    <div className="border-b border-line bg-bg-surface px-4 py-2.5 lg:hidden print:hidden">
      <select value={current} onChange={(e) => router.push(`/dashboard/${scanId}/${e.target.value}`)} className="focus-ring w-full rounded-md border border-line bg-bg-raised px-3 py-2 text-sm text-ink" aria-label="Dashboard section">
