      ...
      <CardBody className="space-y-2.5">
        ...
        {issue.evidence.length > 0 && (
          <ul className="space-y-1 border-l-2 border-line pl-3">
            {issue.evidence.map((e, i) => <li key={i} className="font-mono text-xs text-ink-faint">{e}</li>)}
          </ul>
        )}
...
const PRIORITY_LABEL: Record<Recommendation["priority"], string> = {
  critical: "Priority 1 — Critical", high: "Priority 2 — High", medium: "Priority 3 — Medium", low: "Priority 4 — Low"
};
  ...
  return (
    <Card>
      <CardBody className="space-y-2">
        <div className="flex items-center justify-between">
          <span className="font-mono text-xs uppercase tracking-wide text-ink-faint">#{index + 1} · {PRIORITY_LABEL[recommendation.priority]}</span>
          <RiskBadge severity={recommendation.priority === "critical" ? "critical" : recommendation.priority === "high" ? "high" : recommendation.priority === "medium" ? "medium" : "low"} />
        </div>
        ...
        {recommendation.evidence.length > 0 && (
          <ul className="space-y-1 border-l-2 border-line pl-3">
            {recommendation.evidence.map((e, i) => <li key={i} className="font-mono text-xs text-ink-faint">{e}</li>)}
          </ul>
        )}
      </CardBody>
    </Card>
