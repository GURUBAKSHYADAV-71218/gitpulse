...
/** Testing infrastructure is detected from file tree/deps only. GitPulse never executes repository code. */
export function analyzeTesting(treePaths: string[] | null, declaredDeps: Set<string>): TestingAnalysis {
  if (!treePaths) {
    return {
      detectedFrameworks: [],
      testFileCount: 0,
