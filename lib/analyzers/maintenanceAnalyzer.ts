...
/**
 * Evaluates dependency maintenance signals: staleness, deprecation, and
 * "long version lag". Kept conceptually separate from security: an old
 * package is not automatically a vulnerability, and vice versa.
 */
export function analyzeMaintenance(dependencyAnalysis: DependencyAnalysis): MaintenanceAnalysis {
  ...
  const dataAvailable = dependencies.length === 0 || dependencies.some((d) => !d.registryLookupFailed);
  ...
  return { staleDependencyCount, agingDependencyCount, deprecatedDependencyCount, longVersionLagCount, dataAvailable, notes };
}
