...
/** Builds Issues strictly from real analysis results. Nothing here is hard-coded independent of actual findings. */
export function buildIssues(bundle: AnalysisBundle): Issue[] {
  ...
  if (bundle.securityAnalysis?.dataAvailable) {
    for (const vuln of bundle.securityAnalysis.vulnerabilities) {
      if (vuln.severity === "unknown") continue;
      issues.push({
        ...
        evidence: [`Advisory ${vuln.id}`, vuln.installedVersion ? `Installed version: ${vuln.installedVersion}` : "Installed version unknown", vuln.fixedVersion ? `Fixed in: ${vuln.fixedVersion}` : "No fixed version published yet"],
        recommendation: vuln.fixedVersion ? `Upgrade ${vuln.packageName} to ${vuln.fixedVersion} or later.` : `Review advisory ${vuln.id} for ${vuln.packageName}; no fixed version is published yet.`
      });
    }
  }
  ...
  if (bundle.contributorAnalysis?.dataAvailable && bundle.contributorAnalysis.concentrationRisk === "high") {
    issues.push({
      ...
      description: "Most repository activity is concentrated among a small number of contributors, which can increase continuity risk.",
      evidence: [`Top contributor accounts for ${bundle.contributorAnalysis.topContributorConcentration}% of recorded contributions.`, `Estimated bus factor: ${bundle.contributorAnalysis.estimatedBusFactor}.`],
...
export function buildRecommendations(issues: Issue[]): Recommendation[] {
  ...
  return recommendations.sort((a, b) => PRIORITY_ORDER.indexOf(a.priority) - PRIORITY_ORDER.indexOf(b.priority));
}
