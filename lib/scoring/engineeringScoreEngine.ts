...
// this object only. Weights apply only across categories with available
// data; unavailable categories are excluded and the rest renormalized.
...
export function scoreDependencies(analysis: DependencyAnalysis | null): CategoryScore {
  ...
  if (analysis.majorUpdateCount > 0) {
    score -= clampScore(majorRatio * 40);
  ...
  if (outdatedNonMajor > 0) {
    score -= clampScore((outdatedNonMajor / analysis.totalCount) * 15);
  ...
  if (reasons.length === 0) reasons.push("All analyzed dependencies are current.");

  return { category: "dependency", label: CATEGORY_LABELS.dependency, score: clampScore(score), weight: SCORE_WEIGHTS.dependency, reasons, dataAvailable: true };
}
...
export function scoreSecurity(analysis: SecurityAnalysis | null): CategoryScore {
  ...
  return { category: "security", label: CATEGORY_LABELS.security, score: clampScore(score), weight: SCORE_WEIGHTS.security, reasons, dataAvailable: true };
}
...
export function scoreMaintenance(analysis: MaintenanceAnalysis | null): CategoryScore {
  ...
  return { category: "maintenance", label: CATEGORY_LABELS.maintenance, score: clampScore(score), weight: SCORE_WEIGHTS.maintenance, reasons: analysis.notes, dataAvailable: true };
}

export function scoreRepositoryActivity(metrics: RepositoryMetrics | null, activity: RepositoryActivity | null): CategoryScore {
  ...
  return { category: "repository", label: CATEGORY_LABELS.repository, score: clampScore(score), weight: SCORE_WEIGHTS.repository, reasons, dataAvailable: true };
}
...
export function scoreContributors(analysis: ContributorAnalysis | null): CategoryScore {
  ...
  return { category: "contributor", label: CATEGORY_LABELS.contributor, score: clampScore(score), weight: SCORE_WEIGHTS.contributor, reasons, dataAvailable: true };
}
...
export function scoreTesting(analysis: TestingAnalysis | null): CategoryScore {
  ...
  return { category: "testing", label: CATEGORY_LABELS.testing, score: clampScore(score), weight: SCORE_WEIGHTS.testing, reasons, dataAvailable: true };
}
...
export function scoreDocumentation(analysis: DocumentationAnalysis | null): CategoryScore {
  ...
  return { category: "documentation", label: CATEGORY_LABELS.documentation, score: clampScore(score), weight: SCORE_WEIGHTS.documentation, reasons, dataAvailable: true };
}
...
/** Combines category scores into an overall score, weighted and renormalized across only the categories with available data. Never random, never a placeholder. */
export function calculateOverallScore(categories: CategoryScore[]): EngineeringHealthScore {
  ...
  const overall = totalWeight > 0 ? clampScore(scored.reduce((sum, c) => sum + (c.score as number) * c.weight, 0) / totalWeight) : null;
  const band = classifyBand(overall);

  return { overall, categories, band, bandLabel: BAND_LABELS[band] };
}
