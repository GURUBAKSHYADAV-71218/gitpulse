 ...
 * resolved versions. Never invents vulnerability data: if the request
 * fails for any reason, returns dataAvailable:false with a clear reason
 * rather than an empty "no vulnerabilities" result (which would be
 * indistinguishable from a real clean scan).
 */
export async function fetchVulnerabilities(
  packages: Array<{ name: string; version: string | null }>
): Promise<OsvResult> {
  const queryable = packages.filter((p) => p.version !== null);
  if (queryable.length === 0) {
    return { dataAvailable: true, unavailableReason: null, vulnerabilities: [] };
  }

  const queries = queryable.map((p) => ({ package: { name: p.name, ecosystem: "npm" }, version: p.version }));
  ...
  if (response.status === 429) {
    return { dataAvailable: false, unavailableReason: "Security advisory data unavailable (OSV rate limit exceeded).", vulnerabilities: [] };
  }
  if (!response.ok) {
    return { dataAvailable: false, unavailableReason: `Security advisory data unavailable (OSV returned HTTP ${response.status}).`, vulnerabilities: [] };
  }
  ...
  try {
    ...
    return { dataAvailable: false, unavailableReason: "Security advisory data unavailable (invalid response from OSV).", vulnerabilities: [] };
  }
  ...
  await Promise.all(
    uniqueIds.map(async (id) => {
      try {
        ...
        if (res.ok) detailMap.set(id, (await res.json()) as OsvVulnEntry);
      } catch {
        // A single advisory detail failure degrades that entry, not the whole scan.
      }
    })
  );
  ...
  results.forEach((r, i) => {
    ...
    (r.vulns ?? []).forEach((v) => {
      ...
      const fixedVersion = detail?.affected?.[0]?.ranges?.[0]?.events?.find((e) => e.fixed)?.fixed ?? null;
