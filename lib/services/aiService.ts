...
// findings. If AI_API_KEY is not configured, or the API call fails for any
// reason, GitPulse falls back to a deterministic, template-based summary —
// the rest of the product is completely unaffected either way.
...
function buildDeterministicSummary(input: EngineeringSummaryInput): string {
  ...
  const scoreText =
    overall === null
      ? "could not be fully calculated due to unavailable data"
      : `scored ${overall}/100 (${health.bandLabel.toLowerCase()})`;
  ...
  const weakestText =
    weakest.length > 0 ? `The lowest-scoring areas are ${weakest.map((c) => `${c.label} (${c.score}/100)`).join(" and ")}.` : "";

  const issuesText =
    topIssues.length > 0
      ? `Key findings include ${topIssues.slice(0, 3).map((i) => i.title.toLowerCase()).join("; ")}.`
      : "No high-severity issues were detected in this scan.";

  const recText = topRecommendations.length > 0 ? `The top priority is to ${topRecommendations[0]?.title.toLowerCase()}.` : "";

  return [`${source.label} ${scoreText}.`, weakestText, issuesText, recText].filter(Boolean).join(" ");
}

export async function generateEngineeringSummary(input: EngineeringSummaryInput): Promise<EngineeringSummaryResult> {
  ...
  try {
    ...
    if (!response.ok) return { text: buildDeterministicSummary(input), generatedBy: "deterministic" };

    const json = await response.json();
    const text = json?.content?.find((block: { type: string }) => block.type === "text")?.text;
    if (!text || typeof text !== "string") return { text: buildDeterministicSummary(input), generatedBy: "deterministic" };
